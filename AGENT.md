# AGENT.md — Context Router

> This file is read by every AI harness (Cursor, Claude Code, Codex CLI, Gemini CLI).
> Hard stops and code discipline are inline below — they apply regardless of harness.
> Routing and domain knowledge live in `docs/`. See `docs/SSOT-map.md` for ownership.
> Other harnesses (CLAUDE.md, AGENTS.md) redirect here — this is the single source.

---

## Hard stops — ask before proceeding

- Do not run destructive database operations (`DROP`, `TRUNCATE`, `DELETE` without `WHERE`) without explicit confirmation
- Do not commit or push to `main`/`master` directly
- Do not modify files in `docs/` without being told to update documentation
- Do not expose secrets, tokens, or credentials in code or logs

## Code discipline

- No `any` types (TypeScript projects)
- No commented-out dead code in commits
- No `console.log` left in production code paths
- Every new public function needs a docstring or JSDoc

---

## Context Loading Hierarchy

| Layer | What it provides | Mechanism |
|-------|-----------------|-----------|
| 0 | Hard stops + code discipline | Inline above — visible to every harness |
| 1 | Task-type routing | This file (AGENT.md) — semantic detection → docs |
| 2 | Domain knowledge, auto-attached | `.cursor/context/*.mdc` globs + `.cursor/rules/*.mdc` globs (Cursor only) |
| 3 | Pinpoint doc sections | Specific links in the Layer 1 routing table |

**Load only what the task requires. Do not load the full `docs/` tree.**

---

## Step 1 — Read First (Always)

Before any task, read:
- `docs/architecture/overview.md` — what this system is

---

## Step 2 — Identify Task Domain

| Task type | Load these docs | Suggested model |
|-----------|----------------|-----------------|
| New feature | `docs/architecture/overview.md` + relevant `docs/business/` | Sonnet |
| API change | `docs/api/` | Sonnet |
| Database change | `docs/database/` | Sonnet |
| Bug fix | `docs/architecture/overview.md` + file context | Sonnet / Haiku |
| Deployment | `docs/deployment/` | Haiku |
| Refactor | `docs/architecture/` + `docs/architecture/decisions.md` | Opus |
| Security review | security context + full module | Opus |
| Write tests / docs | file context only | Haiku |
| Unknown | Ask the user before loading anything | — |

**Rule: Load only what the task requires. Do not load the full `docs/` tree.**

## Model Tiers

| Tier | Use when |
|------|----------|
| Opus | Architecture decisions, security review, cross-cutting refactors |
| Sonnet | Feature development, API/DB design, debugging |
| Haiku | Tests, docs, scaffolding, deployment commands |

---

## Step 3 — Apply Rules

Hard stops and code discipline are already inline above (all harnesses).
The rules below are Cursor-specific enhancements (auto-attach via MDC globs):

| Rule file | When it applies |
|-----------|----------------|
| `always.mdc` | Cursor auto-load trigger (points back to this file) |
| `code-style.mdc` | Any code generation or modification |
| `testing.mdc` | Any new function, module, or API endpoint |
| `database.mdc` | Any schema or migration change |
| `security.mdc` | Auth, permissions, data handling |
| `knowledge-placement.mdc` | Any change to docs/, context/, skills/, or LEARNINGS.md |
| `self-check.mdc` | After any change to docs/, rules/, context/, AGENT.md |

---

## Step 4 — Use Skills (SOPs) and Context Providers

Before implementing a known workflow, check generated native skills first:

- Cursor: `.cursor/skills/`
- Codex: `.codex/skills/` if generated; read `.codex/skills/README.md` for the generated index
- Claude Code: `.claude/skills/` if generated; read `.claude/skills/README.md` for the generated index
- Gemini: `skills/` if generated; read `skills/README.md` for the generated index
- OpenCode: `.opencode/skills/` if generated; read `.opencode/skills/README.md` for the generated index

The source of truth for portable skills is `plugins/[name]/skills/`.
If a matching skill exists → follow it exactly. If no skill exists → implement, then consider creating one after.

For domain knowledge orientation, check `.cursor/context/[domain]-context.mdc`.
These auto-attach in Cursor via globs. In other harnesses, use native generated skills when available; explicit context loading is fallback.

- `plugins/` = source of truth for portable AI rules, context providers, and skills
- `.cursor/rules/` = generated Cursor-specific behavior enhancements (hard stops are inline above)
- `.cursor/context/` = generated domain **knowledge** (facts for orientation, summarized from `docs/`)

---

## Step 5 — After Task Completion

Run the post-task reflection:

1. Did we encounter a constraint that should always be enforced?
   → Add to `plugins/project-template/rules/cursor/`, then regenerate Cursor assets

2. Did we complete a repeatable workflow?
   → Add to `plugins/project-template/skills/`, then regenerate harness assets

3. Did facts about the system change?
   → Update the canonical file in `docs/` (check `docs/SSOT-map.md` for owner)

4. Did we make a non-obvious architectural decision?
   → Append to `docs/architecture/decisions.md`

5. Write the raw observation to `LEARNINGS.md` **before** distilling it anywhere.
   Then distill. Propose all additions — do not add silently.

After changing `plugins/`, run:

```bash
python tools/generate_adapters.py --harness cursor
python tools/validate_generated.py
python tools/generate_adapters.py --all --dry-run
python tools/check_template.py
```

---

## Directory Map

> For routing only. For canonical fact ownership, see `docs/SSOT-map.md`.

```
AGENT.md                          ← you are here (canonical router)
CLAUDE.md                         → Claude Code entry point (committed one-line redirect)
AGENTS.md                         → Codex CLI entry point (committed one-line redirect)
GEMINI.md                         → Gemini CLI entry point (committed one-line redirect)
.opencode/AGENTS.md               → OpenCode entry point (committed one-line redirect)
LEARNINGS.md                      → memory distillation staging buffer
README.md                         → human-facing intro, links to docs/

docs/
  SSOT-map.md                     ← domain → canonical file registry
  architecture/
    overview.md                   ← what the system is and how it's structured
    decisions.md                  ← ADR log: why we made key choices
    harness-adapters.md           ← cross-tool adapter contract and capability matrix
  business/                       ← domain logic, business rules, user flows
  api/                            ← API contracts, endpoints, request/response shapes
  database/                       ← schema, ERD, migration conventions
  deployment/                     ← environments, CI/CD, runbooks

plugins/
  project-template/               ← source of truth for portable AI rules/context/skills

.cursor/
  rules/                          ← generated from plugins/project-template/rules/cursor/
    always.mdc                    ← loaded every session, keep it short
    code-style.mdc                ← auto-attaches to source files
    testing.mdc                   ← auto-attaches to test files
    database.mdc                  ← auto-attaches to schema/migration files
    security.mdc                  ← auto-attaches to auth/middleware files
    self-check.mdc                ← auto-attaches to template/docs files; verifies 4-mechanism compliance
  context/                        ← generated from plugins/project-template/context/
    _template.mdc                 ← copy this when adding a new domain context
  skills/                         ← generated from plugins/project-template/skills/
    _template/                    ← copy this directory when creating a new skill
      SKILL.md                    ← frontmatter + core guidance (Tier 1+2)
      references/details.md       ← deep detail, examples (Tier 3, load on demand)

.gitignore
```

---

## SSOT Principle

Every fact exists in **exactly one place**.

- README.md → links to `docs/`, does not duplicate it
- AGENT.md → links to `docs/`, does not duplicate it
- plugins/ → source for portable AI assets, does not own project facts
- If two files say the same thing, one of them is wrong

---

## What NOT to put in this file

- ❌ Tech stack details → `docs/architecture/overview.md`
- ❌ Business rules → `docs/business/`
- ❌ Coding style preferences → `plugins/project-template/rules/cursor/code-style.mdc`
- ❌ How to deploy → `docs/deployment/`
- ❌ Lessons learned → `LEARNINGS.md` first, then distill to `plugins/project-template/rules/` or `plugins/project-template/skills/`
- ❌ Domain facts → `docs/` canonical file (check `docs/SSOT-map.md` for which one)
